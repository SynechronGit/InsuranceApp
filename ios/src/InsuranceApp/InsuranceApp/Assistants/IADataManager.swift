    ...
    fileprivate func sendRequest(_ pRequest:Any!) {
        ...
        do {
            if self.requestType == IARequestType.login {
                ...
                let anSqlQuery :String = "INSERT INTO claims (code, date_of_claim, insurance_type, insured_item_name, insurer, status, incident_date, incident_type, value, photo1, photo2, photo3, policy_number, comments) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
                var aValueArray = Array<AnyObject>()
                ...
                aValueArray.append((NSNull() as AnyObject))
                 _ = try self.executeQuery(anSqlQuery, values: aValueArray)
